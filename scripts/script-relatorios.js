...
//Funções para testar enquando programo
categoriasPorAno()
TotaisMesPorAno()
...
function janelaAnos() {
    ...
    const janela = document.getElementById('janela-escolhas-anos')
    janela.setAttribute('style', 'display: flex')

    //Opções de anos
    let opcoesAnos = document.createElement('p')
    janela.innerHTML = ''
    opcoesAnos.innerHTML = `<button class="botoes">2022</button>`
...
function categoriasPorAno() {
    ...
    tit.innerHTML = `<h3 class="titulos">Totais Categorias</h3>`
...
function TotaisMesPorAno() {
    ...
    const tit = document.createElement('h3')
    tit.innerHTML = `<h3 class="titulos">Totais por Mês</h3>`
    titulo.appendChild(tit)
